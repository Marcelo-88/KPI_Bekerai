...
<style>
    ...
    /* TARJETA NORMAL CON DATO ACTUAL */
    .kpi-card {
        background-color: #FFFFFF; border: 1px solid #E6E1D7; border-radius: 12px;
        padding: 0.75rem 0.9rem; box-shadow: 0 2px 5px rgba(0, 0, 0, 0.04); margin-bottom: 0.75rem;
        min-height: 150px; display: flex; flex-direction: column; justify-content: space-between;
    }
    
    /* TARJETA REMARCADA (FALLBACK A ÚLTIMA SEMANA CON DATO) */
    .kpi-card-fallback {
        background-color: #FFFDF2; border: 2px dashed #E67E22; border-radius: 12px;
        padding: 0.75rem 0.9rem; box-shadow: 0 3px 8px rgba(230, 126, 34, 0.15); margin-bottom: 0.75rem;
        min-height: 150px; display: flex; flex-direction: column; justify-content: space-between;
    }
    
    .kpi-card-header { font-size: 0.85rem; font-weight: 700; color: #2C3E50; text-transform: uppercase; line-height: 1.2; margin-bottom: 0.2rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    ...
    .badge-warning { color: #D35400; font-weight: 700; background: #FDEBD0; padding: 2px 6px; border-radius: 4px; font-size: 0.7rem; display: inline-block; margin-bottom: 3px; }
    ...
    .task-badge { display: inline-block; padding: 2px 8px; border-radius: 12px; font-size: 0.72rem; font-weight: 700; text-transform: uppercase; }
...
def load_data():
    ...
    if not df_kpi_raw.empty:
        # Fila 3 contiene 'QUIEN', 'DEPARTAMENTO', 'MEDIBLES'
        header_idx = None
        ...
        if header_idx is not None:
            ...
            # Detectar unicamente columnas de semanas validas (ej: "Sem 24", "Sem 31")
            val_cols = []
            for c in df_kpi_clean.columns:
                c_str = str(c).strip()
                if c not in id_cols and c_str != '' and c_str.lower() != 'nan' and not c_str.startswith('Unnamed'):
                    if 'sem' in c_str.lower() or any(char.isdigit() for char in c_str):
                        val_cols.append(c)
            ...
            for col in df_kpi_long.columns:
                ...
                elif c_upper == 'MEDIBLES': rename_dict[col] = 'Medible_Base'
            ...
            # Formatear nombre unificado
            def build_full_medible(row):
                m_base = str(row.get('Medible_Base', '')).strip()
                cat = str(row.get('Categoria', '')).strip()
                if cat and cat.lower() != 'nan' and cat != '':
                    return f"{m_base} - {cat}", m_base
                return m_base, m_base

            res = df_kpi_long.apply(build_full_medible, axis=1)
            df_kpi_long['Medible'] = [r[0] for r in res]
            df_kpi_long['Medible_Base'] = [r[1] for r in res]
            ...
            df_kpi_long = pd.DataFrame(columns=['Responsable', 'Departamento', 'Medible', 'Medible_Base', 'Semana', 'Valor'])
    else:
        df_kpi_long = pd.DataFrame(columns=['Responsable', 'Departamento', 'Medible', 'Medible_Base', 'Semana', 'Valor'])

    # 2. PARSE PESTAÑA TAREAS
    try:
...
def render_multi_kpi_chart(df_kpis, kpi_list, title="Comparativa Multi-KPI", height=420, unit_label="Valores"):
    ...
    for idx, kpi in enumerate(existing_kpis):
        ...
        fig.add_trace(
            go.Scatter(
                x=sub_df['Semana'], y=sub_df['Valor'],
                name=f"{kpi} (Eje Der.)" if use_secondary else str(kpi),
                mode='lines+markers',
                line=dict(color=color, width=3),
                marker=dict(size=7, color=color, symbol='circle'),
                hovertemplate=f"<b>{kpi}</b><br>🗓️ %{{x}}<br>📊 Valor: <b>{val_prefix}%{{y:,.2f}}</b><extra></extra>",
                connectgaps=True
            ),
...
menu_option = st.sidebar.radio(
    ...
    ["📊 Dashboards KPIs", "🔀 Comparador KPI vs KPI", "📝 Gestión de Tareas", "🏆 Scorecard & Cumplimiento"]
)
...
if menu_option == "📊 Dashboards KPIs":
    ...
    if not df_kpis.empty and 'Medible' in df_kpis.columns:
        ...
        # Ordenamiento explícito para agrupar tarjetas por Medible Base (Bajas, Ventas, Envio, Produccion)
        base_order = ["BAJAS", "VENTAS", "ENVIO", "PRODUCCION"]
        df_all_selected['Medible_Base_Upper'] = df_all_selected['Medible_Base'].str.upper()
        
        def get_order_score(base_name):
            for idx, b in enumerate(base_order):
                if b in base_name: return idx
            return 99

        df_all_selected['Order_Score'] = df_all_selected['Medible_Base_Upper'].apply(get_order_score)
        df_all_selected = df_all_selected.sort_values(by=['Order_Score', 'Medible'])
        ...
        if len(metrics_list) > 0:
            ...
            for idx, kpi in enumerate(metrics_list):
                ...
                # Buscar dato de la semana seleccionada o retroceder a la última semana con dato
...
# ------------------------------------------
    ...
    if not df_tasks.empty:
        ...
        with f_col1: sel_estados = st.multiselect("📌 Filtrar por Estado:", estados_disponibles, default=estados_disponibles)
            
        deptos_disponibles = sorted(list(df_tasks['Departamento'].dropna().unique()))
        with f_col2: sel_deptos = st.multiselect("🏢 Filtrar por Departamento:", deptos_disponibles, default=deptos_disponibles)
            
        all_resps = sorted(list(df_tasks['Responsable Principal'].dropna().unique()))
        with f_col3: sel_resps = st.multiselect("👤 Filtrar por Responsable:", all_resps, default=[])
        ...
        st.markdown(f"Mostrando **{len(df_filtered)}** de **{len(df_tasks)}** tareas en total.<br>", unsafe_allow_html=True)
        ...
        if not df_filtered.empty:
            ...
            st.markdown(f'<div class="task-container-card">{rows_html}</div>', unsafe_allow_html=True)
        else:
